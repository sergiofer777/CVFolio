...
interface DowngradeToProButtonProps {
  className?: string;
  children: ReactNode;
}

export function DowngradeToProButton({
  className,
  children,
}: DowngradeToProButtonProps) {
  ...
  const handleDowngrade = async () => {
    ...
    try {
      const res = await fetch("/api/billing/downgrade-to-pro", {
        method: "POST",
        ...
        throw new Error(
          data.error ??
            (isEn
              ...
              : "No se pudo abrir el cambio a Pro.")
        );
      }
